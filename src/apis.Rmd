...
Application Programming Interface (APIs) represent a 'gate' or otherwise a platform that enables a client (that is you) to interact with a server (for example [opendata.bristol.gov.uk](https://opendata.bristol.gov.uk/pages/homepage/). According to @amazonWhatAPI:
...
Although some urban data can be downloaded as a tidy\^ \[Every column is a variable; every row is an observation; every cell is a single value. See \[here\])(https://cran.r-project.org/web/packages/tidyr/vignettes/tidy-data.html) for more info\] table in a `.csv` format, most APIs will ship data to you in a `JSON` format.
...
[opendata.bristol.gov.uk](https://opendata.bristol.gov.uk/pages/homepage/) offers a lot of data about Bristol. For this example, let's try to work with [Jobs by Ward (by industry) in Bristol 2020](https://opendata.bristol.gov.uk/explore/dataset/jobs-by-ward-by-industry-in-bristol-2020/information/?disjunctive.ward). As you can see, this is a small data set with $35$ observations representing Bristol's $35$ Wards. You can visualise these data with the `Table` tab and even download a `.csv` with these data. To begin with, do both of these things.
...
So, this is a list. If you need a reminder for what a list is, see [here](http://adv-r.had.co.nz/Data-structures.html). Now the difficult part. Let's try to figure out the structure of this `JSON` formated data.
...
Now you might be lucky and somebody else has already done all the infrastructural / backend work in order for you to easily interrogate an API and access their data. How would this look like? Like another `R` package. Here is a list with a few examples:
