...
Pay attention on the packages `sand` and `igraphdata`. The first one contains all the different network datasets used by [@kolaczyk2020statistical]. Then, the second one contain network data to be used together with the `igraph` package. More information can be found [here](https://cran.r-project.org/web/packages/sand/sand.pdf) and [here](https://cran.r-project.org/web/packages/igraphdata/igraphdata.pdf). Feel free to explore these network data on your own time.
...
Go to `igraph`'s [webpage](https://igraph.org/r/html/latest/) and familiarise yourselves with the syntax of these functions.
