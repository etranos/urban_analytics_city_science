...
```{r}
dat1 <- jsonlite::fromJSON("https://maps2.bristol.gov.uk/server2/rest/services/ext/ll_leisure_and_culture/MapServer/3/query?outFields=*&where=1%3D1&f=geojson", flatten = TRUE)
...
```{r}
dat1 <- dat1$features %>% select(-type, -id, -geometry.type, -geometry.coordinates, -properties.OBJECTID, -`properties.SHAPE.STArea()`, 
...
```{r wrangling}
dat <- dat$features %>% select(-type, -id, -geometry.type, -geometry.coordinates, -properties.OBJECTID, -properties.Shape__Area, 
